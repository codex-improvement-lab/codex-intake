...
To confirm or revoke several already-reviewed IDs together, repeat `--id` and pass the exact read snapshot `--revision`, with `--decision confirm` or `candidate`. Do not infer an all-selection or include newly generated requirements without authorization. The whole batch must validate before publishing one new revision; invalid IDs, duplicate IDs, stale revision and withdrawn/needs-review items reject it. Output never overwrites an existing snapshot. Proofline can read the resulting file directly, so a normalized contract copy is optional.
